...
def get_crossref_doi(doi: str) -> dict | None:
    """Look up a DOI via CrossRef public API (credible, synchronized source)."""
    try:
        url = CROSSREF_API + doi.strip()
        resp = requests.get(url, timeout=15, headers={"User-Agent": "OncoAgent-GBM/1.0 (research; mailto:research@example.edu)"})
        resp.raise_for_status()
        msg = resp.json().get("message", {})
        return {
            "title": msg.get("title", [""])[0] if isinstance(msg.get("title"), list) else msg.get("title", ""),
            "authors": ", ".join([a.get("family", "") + ", " + a.get("given", "") for a in msg.get("author", [])]),
            "journal": msg.get("container-title", [""])[0] if isinstance(msg.get("container-title"), list) else msg.get("container-title", ""),
            "year": msg.get("published-print", msg.get("published-online", {})).get("date-parts", [[""]])[0][0] if msg.get("published-print") else msg.get("published-online", {}).get("date-parts", [[""]])[0][0] if msg.get("published-online") else msg.get("created", {}).get("date-parts", [[""]])[0][0],
            "doi": doi,
...
def get_related_papers(pmid: str, max_results: int = 5) -> list[str]:
    """Retrieve related article PMIDs from PubMed ELink."""
    try:
        params = {
            "dbfrom": "pubmed",
            "db": "pubmed",
            "id": pmid,
            "linkname": "pubmed_pubmed_related",
        }
        resp = requests.get(PUBMED_ELINKS, params=params, timeout=30)
        resp.raise_for_status()
        root = ET.fromstring(resp.content)
        pmids = [id_elem.text for id_elem in root.findall(".//Link/Id") if id_elem.text]
