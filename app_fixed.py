...
def tab_compound_screening():
    ...
    if st.button("Execute Screening", type="primary", use_container_width=True):
        ...
        if results:
            st.markdown("---")
            st.markdown('<div class="section-header">ProTox-3 Toxicity Assessment & Export</div>', unsafe_allow_html=True)
            ...
            for i, result in enumerate(results):
                if result.valid:
                    ...
                    if mol:
                        ...
                        t1.metric("Tox Class", tox_profile.toxicity_class.split("(")[0].strip() if tox_profile.toxicity_class else "N/A")
                        ...
                        t6.metric("Oral Tox", tox_profile.oral_toxicity.split("(")[0].strip() if tox_profile.oral_toxicity else "N/A")
