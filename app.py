...
from agent_evaluator import (
    evaluate_lead, generate_pdf_report, PatientProfile,
    estimate_toxicity, LeadEvaluation,
)
...
def render_header():
    st.markdown(
        ...
        'letter-spacing:0.02em;">'
        'Glioblastoma Multiforme Drug Discovery and Clinical Decision Support Platform</p>'
...
def tab_compound_screening():
    ...
    if st.button("Execute Screening", type="primary", use_container_width=True):
        ...
        if results:
            ...
            with c2:
                from agent_evaluator import generate_pdf_report, evaluate_lead
                for i, result in enumerate(results):
                    if result.valid:
                        ev = evaluate_lead(
        ...
        if st.button("Generate Treatment Plan", type="primary", use_container_width=True):
            ...
            if smiles_for_eval.strip():
                ...
                if compound_result.valid:
                    ...
                    pdf = generate_pdf_report(ev)
                    st.download_button("Download Full Audit Report (PDF)", data=pdf, file_name=f"eval_{patient_id}_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf")
