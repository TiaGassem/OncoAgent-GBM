...
def compute_all_descriptors(mol: Chem.Mol) -> dict:
    ...
    return {
        ...
        "heavy_atoms": mol.GetNumHeavyAtoms(),
        "num_aromatic_rings": len([r for r in mol.GetRingInfo().AtomRings() if all(mol.GetAtomWithIdx(a).GetIsAromatic() for a in r)]),
        "num_aliphatic_rings": len([r for r in mol.GetRingInfo().AtomRings() if not all(mol.GetAtomWithIdx(a).GetIsAromatic() for a in r)]),
        "num_heteroatoms": Lipinski.NumHeteroatoms(mol),
        "num_amide_bonds": len([b for b in mol.GetBonds() if b.GetBondTypeAsDouble() == 1.0 and b.GetBeginAtom().GetAtomicNum() == 7 and b.GetEndAtom().GetAtomicNum() == 6]),
