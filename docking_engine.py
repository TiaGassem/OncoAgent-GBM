...
def smiles_to_pdbqt(smiles: str, output_path: str) -> str:
        ...
        try:
            ...
            if mol_setups:
                ...
                if is_ok:
                    ...
                    # Meeko produced an error message; fall through to manual writer
                    pass
        except Exception:
            # Meeko or RDKit compatibility issue; fall through to manual PDB writer
            pass
    ...
    try:
        ...
        lines = ["REMARK  1 GENERATED BY ONCOAGENT-GBM FALLBACK", "ATOM"]
        for idx in range(mol.GetNumAtoms()):
            atom = mol.GetAtomWithIdx(idx)
            sym = atom.GetSymbol()
            x, y, z = conf.GetAtomPosition(idx)
            atom_name = sym.rjust(4)
            res_name = "LIG"
            res_seq = 1
            chain = "A"
            lines.append(
                f"ATOM  {str(idx+1).rjust(5)} {atom_name:4} {res_name:3} {chain:1}{str(res_seq).rjust(4)}    "
